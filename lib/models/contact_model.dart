...
class Contact {
  ...
  //save contact
  insertContact(dynamic customer) async {
    final db = await dbProvider.database;
    if (customer is List<Map<String, dynamic>>) {
      Batch batch = db.batch(); // Create a batch
      for (var cust in customer) {
       batch.insert('contact', Contact().contactModel(cust),
           conflictAlgorithm: ConflictAlgorithm.replace);
      }
      await batch.commit(noResult: true); // Commit all operations
    } else {
      var response = await db.insert('contact', customer);
      return response;
    }
    await db.rawQuery('SELECT COUNT(*) as count FROM contact');  // Replace 'contacts' with your table name
