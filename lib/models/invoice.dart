...
class InvoiceFormatter {
  ...
  Future<String> generateProductDetails(sellId, context) async {
    ...
    for (int i = 0; i < products.length; i++) {
      ...
      product = '''$product          <tr class="bb-lg">
  ...
  Future<String> generateInvoice(sellId, taxId, context) async {
    ...
    dynamic location;
    for (dynamic element in locations) {
    ...
    dynamic dateTime = DateTime.parse(sells[0]['transaction_date']);
    dynamic date = DateFormat("dd/MM/yyyy").format(dateTime);
    dynamic discountType = sells[0]['discount_type'];
    dynamic discountAmount = sells[0]['discount_amount'];
    ...
    dynamic customerName = customer['name'];
    dynamic customerAddress1 = (customer['address_line_1'] != null)
        ? customer['address_line_1'] + ','
        : '';
    dynamic customerAddress2 = (customer['address_line_2'] != null)
        ? customer['address_line_2'] + ','
        : '';
    dynamic customerCity = (customer['city'] != null) ? customer['city'] + ',' : '';
    dynamic customerState =
    (customer['state'] != null) ? customer['state'] + ',' : '';
    dynamic customerCountry =
    (customer['country'] != null) ? customer['country'] : '';
    dynamic customerMobile = customer['mobile'];
    List paymentList =
    await PaymentDatabase().get(sells[0]['id'], allColumns: true);
    double totalPaidAmount = 0.0;
    String payments = '';
    for (dynamic element in paymentList) {
      String sign;
      ...
      dynamic method = element['method'];
      dynamic paidAmount = element['amount'];
      if (element['amount'] > 0) {
        payments += '''
        <div class="flex-box">
    ...
    returnAmount = num.parse(Helper().formatCurrency(returnAmount));
    ...
    //structure of due
    if (dueAmount > 0) {
      dueAmount =  num.parse(Helper().formatCurrency(dueAmount));
      dueHtml = '''
      <div class="flex-box">
         <p class="width-50 text-left">
