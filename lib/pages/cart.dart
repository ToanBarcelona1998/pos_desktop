...
class CartState extends State<Cart> {
  ...
  Map? argument = {};
  String symbol = '';
  dynamic sellDetail, selectedDiscountType = "fixed";
  ...
  List<String> discountData = ['fixed', 'percentage'];
  ...
  void didChangeDependencies() {
    ...
    cartList();
    taxListMap = [
      {'id': 0, 'name':AppLocalizations.of(context).translate('tax_rate'), 'amount': 0}
    ];
  }
  ...
  Widget inLineTax(index) {
    //['tax_rate_id'], index['dynamiciation_id']
    //taxId, dynamicId
  ...
  Widget inLineDiscount(index) {
    return DropdownButtonHideUnderline(
      child: DropdownButton(
          ...
          items: discountData.map<DropdownMenuItem<String>>((String value) {
  ...
  String calculateInlineUnitPrice(price, taxId, discountType, discountAmount) {
    double subTotal;
    Object? taxAmount;
    for (dynamic value in taxListMap) {
      if (value['id'] == taxId) {
    ...
    if (discountType == 'fixed') {
      dynamic unitPrice = price - discountAmount;
      subTotal = unitPrice + (unitPrice * taxAmount / 100);
    } else {
      dynamic unitPrice = price - (price * discountAmount / 100);
      subTotal = unitPrice + (unitPrice * taxAmount / 100);
    }
  ...
  double calculateSubTotal() {
    ...
    for (dynamic element in cartItems) {
  ...
  double calculateSubtotal(taxId, discountType, discountAmount) {
    ...
    double finalTotal;
    num? taxAmount;
    for (Map<String, dynamic> value in taxListMap) {
      if (value['id'] == taxId) {
        taxAmount = value['amount'];
      }
    }
    ...
    taxAmount ??= 0;
    if (discountType == 'fixed') {
      dynamic total = subTotal - discountAmount;
      finalTotal = total + (total * taxAmount / 100);
    } else {
      dynamic total = subTotal - (subTotal * discountAmount / 100);
      finalTotal = total + (total * taxAmount / 100);
    }
  ...
  getDefaultValues() async {
    dynamic businessDetails = await System().get('business');
    await Helper().getFormattedBusinessDetails().then((value) {
      symbol = "${value['symbol']} ";
    });
    dynamic userDetails = await System().get('loggedInUser');
    setState(() {
