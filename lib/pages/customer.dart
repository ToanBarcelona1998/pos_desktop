...
class CustomerState extends State<Customer> {
  ...
  Widget customerList() {
    String barHint = '${selectedCustomer['name']} (${selectedCustomer['mobile'] ?? ' - '})';
    return Column(
      children: [
        Text(
        ...
        SearchAnchor.bar(
          barHintText:  barHint,
          suggestionsBuilder:
              (BuildContext builderContext, SearchController controller) {
            final keyword = controller.value.text;
            return List.generate(
                customerListMap.length,
                    (index) => CustomerData(
                  name: customerListMap[index]['name'],
                  mobileNumber: customerListMap[index]['mobile'] ?? '-',
                  themeData: themeData,
                  onTap: () async {
                    controller.clear();
                    selectedCustomer = customerListMap[index];
                    Navigator.pop(context);
                    FocusScope.of(builderContext).unfocus();
                  },
                )).where((element) =>
            element.name.contains(keyword) ||
                element.mobileNumber.contains(keyword));
          },
        ),
        /* SearchAnchor.bar(
