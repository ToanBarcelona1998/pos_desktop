...
class LoginCubit extends Cubit<LoginState> {
  ...
  bool _validateOnData() {
    return _formKey.currentState!.validate() && !isLoading;
  }
  ...
  Future<void> _loadAllData(loginResponse, context) async {
    ...
    // Insert token first and verify it's saved correctly
    await System().insertToken(loginResponse['access_token']);
    await Future.delayed(Duration(milliseconds: 500)); // Added: Small delay to ensure DB write completes
    String savedToken = await System().getToken();
    print('Saved Token before fetching contacts: $savedToken'); // Added: Log the saved token for debugging
    if (savedToken.isEmpty) {
      print('Error: Token not saved correctly in DB'); // Added: Error log if token is empty
    ...
    // Fetch contacts after token is confirmed saved
    await CustomerApi().get(); // أضفت هذا: جلب وإدراج العملاء الجدد بعد المسح
